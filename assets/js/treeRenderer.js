...
export class TreeRenderer {
  ...
  renderNodeCards(layoutNodes) {
    ...
    layoutNodes.forEach(item => {
      ...
      const igUrl = this.formatSocialUrl('ig', node.ig);
      const fbUrl = this.formatSocialUrl('fb', node.fb);
      const liUrl = this.formatSocialUrl('li', node.li);
      const sajtUrl = this.formatSocialUrl('sajt', node.sajt);

      let socialHTML = '';
      if (igUrl || fbUrl || liUrl || sajtUrl) {
        const igIcon = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>`;
        const fbIcon = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path></svg>`;
        const liIcon = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg>`;
        const webIcon = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg>`;

        socialHTML = `
          <div class="node-social-row">
            ${igUrl ? `<a href="${igUrl}" target="_blank" class="social-btn ig" title="Instagram" onclick="event.stopPropagation()">${igIcon}</a>` : ''}
            ${fbUrl ? `<a href="${fbUrl}" target="_blank" class="social-btn fb" title="Facebook" onclick="event.stopPropagation()">${fbIcon}</a>` : ''}
            ${liUrl ? `<a href="${liUrl}" target="_blank" class="social-btn li" title="LinkedIn" onclick="event.stopPropagation()">${liIcon}</a>` : ''}
            ${sajtUrl ? `<a href="${sajtUrl}" target="_blank" class="social-btn web" title="Web sajt" onclick="event.stopPropagation()">${webIcon}</a>` : ''}
          </div>
        `;
      }
  ...
  formatSocialUrl(type, val) {
    ...
    if (type === 'li') {
      if (val.startsWith('http')) return val;
      return `https://linkedin.com/${val.startsWith('in/') ? val : 'in/' + val}`;
    }
    if (type === 'sajt') {
      if (val.startsWith('http')) return val;
      return `https://${val}`;
